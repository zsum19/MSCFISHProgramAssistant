...
RSpec.describe Announcement, type: :model do
  ...
  it 'does not save an announcement without title' do
    announcement = described_class.new(title: nil, content: 'test', event_id: 1, member_id: 1)
    expect(announcement).not_to be_valid
  end

  it 'does not save an announcement without content' do
    announcement = described_class.new(title: 'Test', content: nil, event_id: 1, member_id: 1)
    expect(announcement).not_to be_valid
  end

  it 'does not save an announcement without an event id' do
    announcement = described_class.new(title: 'Test', content: 'test', event_id: nil, member_id: 1)
    expect(announcement).not_to be_valid
  end

  it 'does not save an announcement without author id' do
    announcement = described_class.new(title: 'Test', content: 'test', event_id: 1, member_id: nil)
    expect(announcement).not_to be_valid
  end
