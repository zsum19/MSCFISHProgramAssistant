...
RSpec.describe Announcement, type: :model do
  ...
  it 'does not save an announcement without content' do
    announcement = described_class.new(content: nil, event_id: 1, author_id: 1)
    expect(announcement).not_to be_valid
  end

  it 'does not save an announcement without an event id' do
    announcement = described_class.new(content: 'test', event_id: nil, author_id: 1)
    expect(announcement).not_to be_valid
  end

  it 'does not save an announcement without author id' do
    announcement = described_class.new(content: 'test', event_id: 1, author_id: nil)
    expect(announcement).not_to be_valid
  end

  it 'saves an announcement with valid attributes' do
    announcement = described_class.new(content: 'test', event_id: 1, author_id: 1)
